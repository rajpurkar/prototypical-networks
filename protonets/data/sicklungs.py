...
DATA_DIR  = os.path.join(os.path.dirname(__file__), '../../data/pna-nih')
CACHE = { }
...
def convert_tensor(key, d):
    # expects the data to have a single channel
    intermediate = np.array(d[key], np.float32, copy=False)
    if len(intermediate.shape) == 3:
        intermediate = intermediate[:, :, 0]
    d[key] = 1.0 - torch.from_numpy(intermediate).transpose(0, 1).contiguous().view(1, d[key].size[0], d[key].size[1])
    return d
